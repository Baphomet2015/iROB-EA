...
Public Class FormPrincipal
    ...
    ' ----------------------------------------------
    '
    ' Constantes y variables para manejo de Video
    '
    ' ----------------------------------------------
    ...
    Private Sub iniApp(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If (FN_TestFichero(sBBDD) = True) Then
            ...
            If (crearConexion(sBBDD) = True) Then
                ...
                Me.TrackBar_Hor.Value = 90
                Me.TrackBar_Ver.Value = 180
                Me.KeyPreview = True
    ...
    Private Delegate Sub SetNivelXBEE_CallBack(ByVal [xx] As String)

    Private Sub SetNivelXBEE(ByVal sValor As String)

        Dim cAux As Integer

        If Me.CampoNivelXBEE_01.InvokeRequired Then
            Dim d As New SetNivelXBEE_CallBack(AddressOf SetNivelXBEE)
            Me.Invoke(d, New Object() {[sValor]})
        Else
            cAux = CInt(sValor)
            Me.CampoNivelXBEE_01.Text = cAux
            Me.CampoNivelXBEE_02.Text = "Ultima medición: " & Format(Now(), "hh:mm")
            Me.CampoNivelXBEE_01.Refresh()
    ...
    ' ----------------------------------------------
    '
    ' procesaXBEE(ByVal sComando As String)
    '
    ' ----------------------------------------------
    Private Sub procesaXBEE(ByVal sComando As String)

        Dim sAux As String

        ' ----------------------------------------------
        '
        ' . Formato del string de XBEE:
        '
        '          IDE_ROBOT_ID:<xxx>
        '    
        ' . Longitud del string: 9 Caracteres
        '
        ...
        sAux = Mid$(sComando, Len(IDE_ROBOT_ID) + 3, 3)
    ...
    ' ---------------------------------------
    ' TrackBar_Hor_MouseUp
    ' ---------------------------------------

    Private Sub TrackBar_Hor_MouseUp(ByVal sender As System.Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles TrackBar_Hor.MouseUp

        Dim cValor As Integer
        Dim sComando As String

        cValor = 180 - CInt(Me.TrackBar_Hor.Value)
        sComando = "MT_HORPA1" & Format(cValor, "000")
        Me.Campo_TX.Text = enviarComando(sComando)

    End Sub

    ' ---------------------------------------
    ' TrackBar_Ver_MouseUp
    ' ---------------------------------------

    Private Sub TrackBar_Ver_MouseUp(ByVal sender As System.Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles TrackBar_Ver.MouseUp

        Dim cValor As Integer
        Dim sComando As String

        cValor = 180 - CInt(Me.TrackBar_Ver.Value)
        sComando = "MT_VERPA1" & Format(cValor, "000")
        Me.Campo_TX.Text = enviarComando(sComando)
    ...
    ' ---------------------------------------
    '
    '  FUNCION ASOCIADA AL SISTEMA DE VIDEO
    '
    ...
    '  FUNCION ASOCIADA AL SISTEMA DE VIDEO
    '
    ' btnSave_Click( ByVal sender As System.Object,
    '                ByVal e      As System.EventArgs
    ...
    '  FUNCION ASOCIADA AL SISTEMA DE VIDEO
    '
    ' btnInfo_Click( ByVal sender As System.Object,
    '                ByVal e As System.EventArgs
    '              ) Handles btnInfo.Click
    '
    ' ---------------------------------------
    ...
    '  FUNCION ASOCIADA AL SISTEMA DE VIDEO
    '
    ' btnStop_Click( ByVal sender As System.Object,
    '                ByVal e      As System.EventArgs
    ...
    Private Sub OpenPreviewWindow()
        ...
        hHwnd = capCreateCaptureWindowA(DeviceID.ToString, WS_VISIBLE Or WS_CHILD, 0, 0, 1280, 1024, picCapture.Handle.ToInt32, 0) ' Open Preview window in picturebox
        '
        ' Connect to device
        '
        If (SendMessage(hHwnd, WM_CAP_DRIVER_CONNECT, DeviceID, 0)) Then
            ...
            SetWindowPos(hHwnd, HWND_BOTTOM, 0, 0, picCapture.Width, picCapture.Height, SWP_NOMOVE Or SWP_NOZORDER) ' Resize window to fit in picturebox
