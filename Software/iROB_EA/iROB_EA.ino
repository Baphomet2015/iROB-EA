...
void setup(void)
{
  ...
  // Funcion:  INT_rtc_SQW()
  // Variable: NO se utiliza esta funcionalidad, reservada por
  //           si se necesita
  // ---------------------------------------------------------
  ...
  switch( GLOBAL_FlgStatusPC )
        {
          case ( IDE_STATUS_PC_INI_ON ):
               { // ----------------------------------------
                 ...
                 // ----------------------------------------
                 FNG_DisplayMsgPROGMEM(IDE_MSG_DISPLAY_INI_PC,0);
                 pc_ON();
                 break;
               }
...
void pc_ON(void)
{
  ...
  // 5 - Inicializa el Timer de espera a que el PC comunique 
...
void pc_OFF(void)
{
  if ( getIcc5VP()==true )
     { // ----------------------------------------------------
       ...
       // SOLO se debe actuar sobre este pulsador ahora  si el
...
void procesoActivo(void)
{
  ...
  if ( GLOBAL_FlgModoAvance==IDE_MODO_AVANCE_CON_PROTECCION )
     { // ------------------------------------------------------
       ...
       //
       // 1 - Deteccion de suelo en modo avance y paro si existe
       //     peligro
       //
       // 2 - Deteccion de objetos (ultrasonidos) en modo avance
       //     y paro si existe peligro
       //
       // 2 - Deteccion de  obstaculo  (sobre consumo)  en  modo
       //     avance y paro si existe peligro
       //
       // ------------------------------------------------------
       
       superficie = sensor_USS.get_Superficie();
       if ( superficie<2 )
          {
